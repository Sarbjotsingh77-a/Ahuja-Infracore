...
(function () {
  ...
  const GOOGLE_SHEET_URL = 'https://script.google.com/macros/s/AKfycbyZ3vYGmvBE6lKk7wFSacw-TR3ebF-Dow-4GCslFaQyfuGvsUHCNJSHNaqo7ZvokIQq/exec'
  ...
  function injectModal() {
    ...
    return fetch(PARTIAL_PATH)
      ...
      .catch((err) => console.error('contact-modal: could not load partial', err));
  }
  ...
  function findTriggers() {
    ...
    document.querySelectorAll('[data-open-contact-modal]').forEach((el) => triggers.add(el));
    document.querySelectorAll('a, button').forEach((el) => {
      if (el.textContent.trim().toLowerCase() === 'get in touch') triggers.add(el);
    });
  ...
  function renderBreadcrumb() {
    ...
    breadcrumbEl.querySelectorAll('button[data-crumb]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (btn.dataset.crumb === 'subcategories') openCategory(currentCat.slug, currentCat.title);
      });
    });
  }
  ...
  function renderCategories() {
    ...
    listEl.innerHTML = entries.map(([slug, cfg]) => `
  ...
  function openCategory(slug, title) {
    ...
    fetchCategoryData(slug)
      .then(({ data }) => {
        ...
        `).join('');
        listEl.querySelectorAll('.cm-list-item').forEach((row) => {
          row.addEventListener('click', () => openSubcategory(subs.find(s => s.id === row.dataset.id)));
        });
      })
      .catch((err) => {
        console.error(err);
  ...
  function renderProducts() {
    ...
    listEl.querySelectorAll('.cm-list-item').forEach((row) => {
      row.addEventListener('click', () => toggleProduct(currentProducts[Number(row.dataset.index)]));
    });
  }

  function toggleProduct(p) {
    const key = (s) => s.categoryTitle === currentCat.title && s.subcategoryName === currentSub.name && s.productName === p.name;
  ...
  function renderChips() {
    if (!selected.length) { chipsEl.innerHTML = ''; return; }
  ...
  function handleSubmit(e) {
    ...
    const requirementsText = selected.length
      ? selected.map((s) => `- ${s.productName} (${s.subcategoryName} > ${s.categoryTitle})`).join('\n')
      : 'Not specified';
    ...
    fetch(GOOGLE_SHEET_URL, {
      ...
      body: JSON.stringify({ name, phone, email, requirements: requirementsText, comments: comments || 'None' }),
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  function showStatus(msg, isError) {
    ...
    statusEl.className = 'cm-status ' + (isError ? 'cm-status-error' : 'cm-status-success');
  }
